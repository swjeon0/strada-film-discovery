import {RECOMMENDATION_COUNT} from '../recommendation-policy';
...
export async function resolveCandidates(candidates:{title:string,year:number,director:string}[],budget:Budget){
 ...
 if(config().tmdb){const failures:unknown[]=[];await mapLimited(pending,3,async i=>{try{const film=await resolveCandidate(candidates[i].title,candidates[i].year,candidates[i].director,budget);if(film)found.set(i,film);}catch(error){failures.push(error);}});if(!found.size&&failures.length)throw failures[0];}
  ...
  try{
   const titles=[...new Set(pending.flatMap(i=>[candidates[i].title,`${candidates[i].title} (${candidates[i].year} film)`,`${candidates[i].title} (film)`]))].slice(0,50);
   const data=await wikiApi('en.wikipedia.org',{action:'query',titles:titles.join('|'),redirects:'1',prop:'pageprops',ppprop:'wikibase_item'},budget);
   const qids=[...new Set<string>((data.query?.pages??[]).map((p:any)=>p.pageprops?.wikibase_item).filter(Boolean))];
   const movies=await filmsFromQids(qids,budget);
   for(const i of pending){const c=candidates[i];const hits=movies.filter(f=>[f.title,f.originalTitle??'',...f.aliases??[],f.wikiEn?.replace(/\s*\([^)]*film[^)]*\)$/i,'')??''].some(title=>titleMatches(title,c.title))&&Math.abs(f.year-c.year)<=1&&f.director.split(',').some(d=>normalizeName(d)===normalizeName(c.director)));if(hits.length===1){const film=canonicalFilm(hits[0]);found.set(i,film);}}
  }catch{}
  // A few unusual database aliases may need the broader Wikidata title search.
  const unresolved=pending.filter(i=>!found.has(i));
  if(new Set([...found.values()].map(f=>f.id)).size<RECOMMENDATION_COUNT)await mapLimited(unresolved.slice(0,6),2,async i=>{if(budget.remaining<4)return;try{const f=await resolveCandidate(candidates[i].title,candidates[i].year,candidates[i].director,budget);if(f)found.set(i,f);}catch{}});
 }
