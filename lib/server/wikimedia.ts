...
const FILM_TYPES=new Set(['Q11424','Q24862','Q24869','Q202866','Q506240','Q20667187','Q29168811']);
...
async function mapEntities(es:Record<string,any>,budget?:Budget){const rows=Object.values(es).filter(isMovieEntity);const relatedIds=rows.flatMap(e=>[...qids(e,'P57'),...qids(e,'P495')]);const related=relatedIds.length?(await wikiApi('www.wikidata.org',{action:'wbgetentities',ids:[...new Set(relatedIds)].join('|'),props:'labels',languages:'en|mul|ko'},budget)).entities??{}:{};return rows.map(e=>mapEntity(e,related)).filter((f):f is Film=>!!f);}
export async function searchWikimedia(query:string,language:Language,budget?:Budget):Promise<Film[]>{
 const data=await wikiApi('www.wikidata.org',{action:'wbsearchentities',search:query,language,uselang:language,limit:'16',type:'item'},budget);const ids=(data.search??[]).map((x:any)=>x.id).filter((id:string)=>/^Q\d+$/.test(id));if(!ids.length)return [];const mapped=await mapEntities(await entities(ids,budget),budget);const index=new Map(ids.map((id:string,i:number)=>[id,i]));return mapped.sort((a,b)=>(index.get(a.wikidataId!) as number)-(index.get(b.wikidataId!) as number)).slice(0,8);
}
