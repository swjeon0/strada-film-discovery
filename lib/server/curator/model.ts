...
export type CuratorRepairOptions = {
  model: string;
  timeoutMs: number;
  maxOutputTokens: number;
};
export type CuratorCallOptions = {
  model: string;
  reasoning: "none" | "low" | "medium" | "high" | "xhigh" | "max" | null;
  timeoutMs: number;
  maxOutputTokens: number;
  promptVersion?: "v1" | "v2";
  repair?: CuratorRepairOptions;
};
export type CuratorCallResult = {
...
function contextTuple(
  ...
  passage: CuratorRequest["context"]["passages"][number],
  index: number,
) {
  if (passage.observation)
    return [
      ...
      passage.filmIds,
      {
        reading: passage.observation,
        boundary: passage.boundary,
        kind: passage.connectionKind,
        films: passage.relatedFilms?.map((film) => [
          film.title,
          film.year,
          film.director,
        ]),
  ...
  const selectedMentions = passage.filmIds.filter((id) =>
    selectedIds.has(id),
  ).length;
  const relational =
...
export async function curateOnce(
  request: CuratorRequest,
  ...
  if (/(?:^|[-_.])sol(?:$|[-_.])/i.test(options.model))
    throw new AppError(
      "MODEL_NOT_ALLOWED",
      "Sol is excluded from the STRADA curator experiment.",
      400,
    );
  const key = config().openai;
  if (!key)
    throw new AppError("SETUP_REQUIRED", "OPENAI_API_KEY is missing.", 503);
  ...
  try {
    response = await fetcher("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        text: {
          format: {
            type: "json_schema",
            name: "strada_curator_v1",
            strict: true,
            schema: CURATOR_V1_OUTPUT_JSON_SCHEMA,
          },
        },
        input: [
          {
            ...
            content: curatorPrompt(
              "list",
              options.promptVersion === "v1" ? SYSTEM_V1 : SYSTEM_V2,
            ),
          },
          {
            role: "user",
            content: JSON.stringify({
              ...
              s: request.selected.map(
                (
                  ...
                  (
                    ...
                    0,
                    Math.min(700, Math.floor(2800 / request.selected.length)),
                  ),
                ],
              ),
              ...
              f: [
                ...new Map(
                  [...request.selected, ...(request.forbiddenFilms ?? [])].map(
                    (film) => [film.id, [film.title, film.year, film.director]],
                  ),
                ).values(),
              ],
    ...
    if (stageSignal.aborted)
      throw new AppError(
        "TIMEOUT",
        "The stage-1 curator exceeded its deadline.",
        504,
      );
    throw error;
  }
  ...
  if (data.status !== "completed")
    throw new AppError(
      "INCOMPLETE",
      "The stage-1 curator did not return a complete result.",
    );
  ...
  try {
    ...
    throw new AppError(
      "INVALID_CURATOR_OUTPUT",
      "The stage-1 curator returned invalid JSON.",
    );
  }
  let output: CuratorOutput;
  try {
    output = validateCuratorOutput(decodeCuratorWire(parsed, request), request);
  } catch (error) {
    const code =
      error instanceof Error && /^CURATOR_[A-Z_]+$/.test(error.message)
        ? error.message
    ...
    throw new AppError(
      code,
      "The stage-1 curator output violated the evidence or film contract.",
    );
  }
...
export async function repairCuratorOnce(
  request: CuratorRequest,
  original: CuratorOutput,
  indexes: number[],
  options: CuratorRepairOptions,
  signal: AbortSignal,
  fetcher: CuratorFetch = fetch,
): Promise<CuratorCallResult> {
  if (!indexes.length || indexes.length > original.recommendations.length)
    throw new AppError(
      "INVALID_CURATOR_OUTPUT",
      "The curator returned invalid film identities.",
    );
  if (/(?:^|[-_.])sol(?:$|[-_.])/i.test(options.model))
    throw new AppError(
      "MODEL_NOT_ALLOWED",
  ...
  try {
    response = await fetcher("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        text: {
          format: {
            type: "json_schema",
            name: "strada_curator_repair",
            strict: true,
            schema: curatorRepairJsonSchema(indexes.length),
          },
        },
        input: [
          { role: "system", content: curatorPrompt("repair", REPAIR_SYSTEM) },
          {
            role: "user",
            content: JSON.stringify({
              g: request.language,
              ...
              r: original.recommendations
                .filter((_, index) => !indexes.includes(index))
                .map(({ title, year, director, anchorIds, connection }) => [
                  title,
                  year,
                  director,
                  anchorIds.map((id) =>
              ...
              bad: indexes.map((index) => {
                const item = original.recommendations[index];
                return [
                  index,
                  item.title,
                  item.year,
                  item.director,
                  item.anchorIds.map((id) =>
                    request.selected.findIndex((film) => film.id === id),
                  ),
                  item.connection,
                ];
    ...
    if (stageSignal.aborted)
      throw new AppError(
        "TIMEOUT",
        "The curator identity repair exceeded its deadline.",
        504,
      );
    throw error;
  }
    ...
    throw new AppError(
      response.status === 429
        ...
        : response.status === 401
          ? "SETUP_REQUIRED"
          : "CURATOR_PROVIDER_ERROR",
      "The curator identity repair failed.",
      response.status === 429 ? 429 : 502,
    );
  const data = (await response.json()) as ProviderResponse;
  if (data.status !== "completed")
    throw new AppError(
      "INCOMPLETE",
      "The curator identity repair did not return a complete result.",
    );
  let parsed: unknown;
  try {
    ...
    throw new AppError(
      "INVALID_CURATOR_OUTPUT",
      "The curator identity repair returned invalid JSON.",
    );
  }
  let output: CuratorOutput;
  try {
    output = applyCuratorRepairs(parsed, request, original, indexes);
  } catch {
    throw new AppError(
      "INVALID_CURATOR_OUTPUT",
      "The curator identity repair violated the film contract.",
    );
  }
  const model = data.model ?? options.model;
  return {
