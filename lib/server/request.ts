...
// A conservative local guard; Sites owner-only access is the deployment boundary.
// A public deployment also needs a shared platform rate limit.
const callers=new Map<string,{started:number,count:number}>();let active=0;
export function reserveResearch(request:Request){
 const now=Date.now();const key=request.headers.get('cf-connecting-ip')??'local';const previous=callers.get(key);const bucket=previous&&now-previous.started<600000?previous:{started:now,count:0};
