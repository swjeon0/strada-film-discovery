...
export function checkOrigin(r:Request){
 const origin=r.headers.get('origin');if(!origin)return;
 ...
 const url=new URL(r.url),host=r.headers.get('host')||url.host,forwardedProtocol=r.headers.get('x-forwarded-proto');
 const protocol=forwardedProtocol==='https'||forwardedProtocol==='http'?`${forwardedProtocol}:`:url.protocol;
 if(origin!==`${protocol}//${host}`)throw new AppError('INVALID_ORIGIN','This request must come from STRADA.',403);
}
