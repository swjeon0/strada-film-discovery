...
const schemaKeys = [
  'sourceIds', 'sourceId', 'sourceIndices', 'contextSourceIndices', 'sources',
  'candidates', 'connections', 'anchorId', 'anchorIndex', 'anchorTitle',
  'inferenceWhy', 'inferenceWhyKo', 'whyKo', 'reasonKo', 'filmForm', 'watchFor',
  'verifiedCriticalSources', 'knownConnections', 'knownSources',
].join('|');
const quote = String.raw`(?:\\*["'“”‘’]|\\u(?:0022|0027|201[89cd]))`;
const space = String.raw`(?:\s|\\[nrt])*`;
const quotedField = new RegExp(`${quote}${space}(?:${schemaKeys})${space}${quote}${space}(?::|：|\\\\u003[aA])`, 'iu');
const unquotedField = new RegExp(String.raw`[}\],{]${space}(?:${schemaKeys})${space}:`, 'iu');
...
function artifactOffset(text: string): number {
  ...
  const offsets = [quotedField.exec(text)?.index, unquotedField.exec(text)?.index,
    closingTail.exec(text)?.index].filter((i): i is number => i !== undefined);
...
function trimArtifactBoundary(text: string): string {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (result.endsWith('”') && (result.match(/”/gu)?.length ?? 0) > (result.match(/“/gu)?.length ?? 0)) {
      result = result.slice(0, -1).replace(/\\+$/u, '');
    } else if (result.endsWith('’') && (result.match(/’/gu)?.length ?? 0) > (result.match(/‘/gu)?.length ?? 0)) {
      result = result.slice(0, -1).replace(/\\+$/u, '');
    } else if (result.endsWith('"') && (result.match(/(?<!\\)"/gu)?.length ?? 0) % 2 === 1) {
      result = result.slice(0, -1).replace(/\\+$/u, '');
