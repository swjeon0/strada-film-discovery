...
export const collection:Film[]=raw.films.map(f=>({...metadata.find(m=>m.key===f.key),wikidataId:metadata.find(m=>m.key===f.key)?.qid,id:f.key,title:f.title,year:f.year,director:f.director,country:f.country,runtime:'runtime' in f?f.runtime:undefined,aliases:f.aliasesKo,sourceIds:f.sourceIds,poster:`/posters/${f.key}.jpg`}));
export const sourceLibrary:Source[]=raw.sources.map(s=>({...s,summaryKo:(summariesKo as Record<string,string>)[s.id]})) as Source[];
...
export function recommendCollection(seeds:Film[],trail:Film[],seenIds:string[]=[]):Batch{
 ...
 const ranked=rankRecommendations(recs,seeds,trail,seenIds);const used=new Set(ranked.flatMap(r=>r.sourceIds));return {mode:'collection',recommendations:ranked,sources:sourceLibrary.filter(s=>used.has(s.id))};
}
