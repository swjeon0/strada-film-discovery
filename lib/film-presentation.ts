...
export function mergeFilmPresentation(selected:Film,details:Film):Film{
 ...
 return {...details,poster:safePoster(selected.poster)||safePoster(details.poster),titleKo,titleKoSource:details.titleKoSource||selected.titleKoSource||(titleKo&&selected.id.startsWith('tmdb:')?`https://www.themoviedb.org/movie/${selected.id.slice(5)}`:undefined)};
}
...
export function retainSelectedPosters(films:Film[],selected:Film[]):Film[]{
 ...
 return films.map(f=>{const previous=chosen.get(f.id);return previous?mergeFilmPresentation(previous,f):f;});
}
