...
export function discoveryHistoryContext(
  session: Pick<Session, "snapshots" | "cursor" | "archivedSeenIds">,
  continuing: boolean,
): { discoveredFilms: DiscoveredFilm[]; seenIds: string[] } {
  ...
  if (films.size > MAX_DISCOVERED_FILMS || seen.size > MAX_SEEN_FILMS)
    throw new Error(
