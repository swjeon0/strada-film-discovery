...
const quality={direct_connection:1,grounded_interpretation:.8,curatorial_association:.62,ai_inference:.38};
export function connectionScore(rec:Recommendation,seeds:Film[],trail:Film[]){
 if(rec.contextScope==='discovery'&&rec.connections.every(c=>c.relation==='ai_inference'))return quality.ai_inference;
...
export function rankRecommendations(recs:Recommendation[],seeds:Film[],trail:Film[],seenIds:string[]=[],limit=12){
 ...
 while(unique.length&&chosen.length<limit){unique.sort((a,b)=>{const value=(r:Recommendation)=>{const relevance=(base.get(r.film.id)??0)/max;const redundancy=chosen.length?Math.max(...chosen.map(s=>similarity(r.film,s.film))):0;return relevance*(1-.22*redundancy)-(seen.has(r.film.id)?.025:0);};const supported=(r:Recommendation)=>r.connections.some(c=>c.relation!=='ai_inference');return Number(supported(b))-Number(supported(a))||value(b)-value(a)||a.film.id.localeCompare(b.film.id);});chosen.push(unique.shift()!);}
 return chosen;
}
