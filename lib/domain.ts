...
export const RunDiagnosticsSchema=z.object({engine:z.string(),model:z.string(),corpusVersion:z.string(),documents:z.number(),observations:z.number(),retrievedDocuments:z.number(),retrievedPassages:z.number(),usedDocuments:z.number(),coveredFilmIds:z.array(z.string()),repairAttempted:z.boolean(),clientMs:z.number().optional(),timings:z.record(z.number()).optional()});
export type RunDiagnostics=z.infer<typeof RunDiagnosticsSchema>;
export const SnapshotSchema=z.object({id:z.string(),createdAt:z.string(),seeds:z.array(FilmSchema).min(1).max(8),trail:z.array(FilmSchema).max(30),recommendations:z.array(RecommendationSchema).min(1).max(12),sources:z.array(SourceSchema).max(48),language:z.enum(['en','ko']).optional(),action:ResearchIntentSchema.optional(),diagnostics:RunDiagnosticsSchema.optional(),evidenceStatus:z.literal('historical').optional(),preparationToken:z.string().max(220000).optional(),reserveCount:z.number().int().min(0).max(32).optional(),notice:z.literal('partial').optional(),mode:z.enum(['collection','live'])});
...
export function commitSnapshot(s:Session,snapshot:Snapshot,newTrail:boolean):Session{
 ...
 if(snapshot.mode==='live'&&snapshot.recommendations.length!==12)throw new Error('A live recommendation batch must contain exactly twelve films.');
