...
const EXTRACTOR_VERSION = 3,
  ...
  TIMEOUT_MS = 18_000,
  CACHE_MS = 7 * 86_400_000;
const PDF_PYTHON =
  process.env.KNOWLEDGE_PDF_PYTHON ??
  process.argv
    .find((arg) => arg.startsWith("--pdf-python="))
    ?.slice("--pdf-python=".length) ??
  "python3";
...
type QuoteResult = {
  ...
  offset?: number;
  offsetBasis?: "extracted_text" | "normalized_text";
  locator: string;
};
type AuditRow = {
  documentId: string;
...
export function extractAuditText(html: string, requestedFragment = "") {
  ...
  function findLinks(node: DefaultTreeAdapterTypes.Node) {
    if ("tagName" in node && node.tagName === "a") {
      ...
      if (href?.startsWith("#") && href.length > 1)
        linkedSections.add(href.slice(1));
    }
    if ("childNodes" in node)
      for (const child of node.childNodes) findLinks(child);
  }
  findLinks(document);
  function visit(node: DefaultTreeAdapterTypes.Node) {
    ...
    if ("tagName" in node) {
      const hidden = node.attrs.some(
        (a) =>
          a.name === "hidden" ||
          (a.name === "aria-hidden" && a.value === "true"),
      );
      const linked = node.attrs.some(
        (a) => a.name === "id" && linkedSections.has(a.value),
      );
...
export function quoteCheck(
  text: string,
  passage: RecordInput["passages"][number],
): QuoteResult {
  ...
  return offset >= 0
    ? {
        ...base,
        status: "matched",
        matchMode: "typography_normalized",
        offset,
        offsetBasis: "normalized_text",
      }
    : { ...base, status: "quote_missing" };
}
export function sourceUrlAllowed(raw: string) {
  try {
    ...
    return (
      ...
      !isIP(host.replace(/^\[|\]$/g, "")) &&
      host.includes(".") &&
      !/(?:^|\.)(?:localhost|local|internal|test|invalid|example|nip\.io|sslip\.io)$/.test(
        host,
      )
    );
  } catch {
...
async function pdfText(pdfPath: string, txtPath: string) {
  try {
    ...
    await execFileAsync(PDF_PYTHON, ["-c", code, pdfPath, txtPath], {
      timeout: 15_000,
      maxBuffer: 1_000_000,
    });
  }
...
function isChallenge(text: string) {
  return /(?:just a moment|checking your browser|verify you are human|enable javascript and cookies to continue|access denied|attention required!.*cloudflare)/i.test(
    text.slice(0, 2500),
  );
}
...
async function fetchOnce(raw: string): Promise<FetchResult> {
  ...
  try {
    for (let redirects = 0; redirects < 5; redirects++) {
      if (!sourceUrlAllowed(url))
        return {
          ...
          status: "invalid_url",
          reason:
            "Only public HTTPS publisher URLs without credentials or custom ports are accepted.",
        };
      const response = await fetch(url, {
        ...
        headers: {
          "User-Agent":
            "STRADA-source-audit/1.0 (bounded citation verification)",
      ...
      if (!response.ok) {
        ...
        return {
          ...
          status: [401, 403, 429].includes(response.status)
            ? "blocked"
            : "http_error",
          reason: `Publisher returned HTTP ${response.status}. Quote presence was not assessed.`,
        };
      }
      ...
      if (
        ...
        const pdfPath = join(cacheRoot, `${hash(body)}.pdf`),
          txtPath = join(cacheRoot, `${hash(body)}.txt`);
        await mkdir(cacheRoot, { recursive: true });
        await writeFile(pdfPath, body);
        try {
          text = await pdfText(pdfPath, txtPath);
        } catch {
          return {
            ...meta,
            status: "pdf_not_checked",
            reason:
              "Neither pdftotext nor the configured Python/pypdf converter completed. Original PDF fetched, but quotes were not checked. Set KNOWLEDGE_PDF_PYTHON or --pdf-python to an installed interpreter.",
          };
        }
      } else if (contentType.includes("html"))
        text = extractAuditText(
          body.toString("utf8"),
          new URL(raw).hash.slice(1),
        );
        ...
        return {
          ...
          reason:
            "Response is not HTML, plain text, or PDF. Quotes were not checked.",
        };
      if (isChallenge(text))
        return {
          ...meta,
          status: "blocked",
          reason:
            "Publisher delivered a browser/access challenge. Quote presence was not assessed.",
        };
      if (text.trim().length < 80)
        return {
          ...meta,
          status: "unreadable",
          reason: "Too little extracted text to assess quote presence.",
        };
    ...
    return {
      ...
      url,
      status: "http_error",
      reason: "Redirect limit reached. Quotes were not checked.",
    };
  } catch (error) {
    return {
      ...base,
      ...
      reason:
        error instanceof Error && error.message === "response_too_large"
          ? `Response exceeds ${MAX_BYTES} bytes. Quotes were not checked.`
          : "Network request failed or timed out. Quote presence was not assessed.",
    };
  }
}
async function readSource(url: string, refresh: boolean): Promise<FetchResult> {
  ...
  if (!refresh)
    try {
      ...
      if (
        ...
        cached.text &&
        hash(cached.text) === cached.textHash &&
        Date.now() - Date.parse(cached.checkedAt) < CACHE_MS
      )
  ...
  if (
    ...
    (result.status === "http_error" && (result.httpStatus ?? 0) >= 500)
  ) {
    await new Promise((resolve) => setTimeout(resolve, 650));
    result = await fetchOnce(url);
  }
  if (result.status === "readable")
    await atomic(cachePath, { ...result, extractorVersion: EXTRACTOR_VERSION });
...
export async function auditRecord(
  ...
  const quotes =
    result.status === "readable"
      ? record.passages.map((p) => quoteCheck(result.text!, p))
      : record.passages.map((p) => ({
          passageId: p.id,
          locator: p.locator,
  ...
  return {
    ...
    status:
      result.status === "readable"
        ? quotes.length && quotes.every((q) => q.status === "matched")
          ? "matched"
          : "quote_missing"
        : result.status,
    ...
    reason:
      result.reason ??
      (quotes.some((q) => q.status === "quote_missing")
        ? "Quote not located in extracted response. Review changed publisher wording or omitted dynamic/transcript content."
        : undefined),
    quotes,
  };
}
async function main() {
  ...
  if (new Set(records.map((r) => r.id)).size !== records.length)
    throw new Error("Duplicate document IDs; audit stopped.");
  const refresh = process.argv.includes("--refresh"),
    results: AuditRow[] = new Array(records.length);
  let cursor = 0;
  await Promise.all(
    Array.from({ length: CONCURRENCY }, async () => {
      while (cursor < records.length) {
        ...
        results[index] = await auditRecord(record, (url) =>
          readSource(url, refresh),
        );
        console.log(`${record.id}: ${results[index].status}`);
      }
    }),
  );
  ...
  await atomic(resolve("research/knowledge/source-audit.json"), {
    ...
    corpusRecordsHash: hash(JSON.stringify(records)),
    documentCount: records.length,
    counts,
    notes: [
      "Quote matching checks text presence only, not interpretive entailment or rights clearance.",
      "Normalized offsets refer to typography-normalized extracted text; exact offsets refer to extracted text.",
      "Cached sources retain the original fetch checkedAt. Full source text is only in ignored work/knowledge/source-cache.",
    ],
    documents: results,
  });
  console.log(
