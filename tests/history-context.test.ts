...
const snapshot = (step: number): Snapshot => ({
  ...
  recommendations: Array.from({ length: 12 }, (_, n) => ({
    film: {
      id: `film-${step * 12 + n}`,
      title: `Film ${step * 12 + n}`,
      year: 2000,
      director: "Director",
      poster: "/posters/unused.jpg",
      synopsisEn: "Omitted from the compact context.",
    },
    connections: [
      {
        anchorId: "seed",
        anchorTitle: "Starting film",
...
test("discovery history retains all 372 possible films with compact database identity only", () => {
  ...
  assert.deepEqual(context.discoveredFilms[0], {
    id: "film-0",
    title: "Film 0",
    year: 2000,
    director: "Director",
...
test("undo retains all previously displayed films including future snapshots without double counting", () => {
  ...
  assert.deepEqual(
    context.seenIds,
    context.discoveredFilms.map((f) => f.id),
  );
...
test("starting a new path never imports films from an unrelated saved path", () => {
  assert.deepEqual(
    discoveryHistoryContext({ snapshots: [snapshot(0)], cursor: 0 }, false),
    { discoveredFilms: [], seenIds: [] },
  );
  assert.deepEqual(
    discoveryHistoryContext({ snapshots: [], cursor: -1 }, true),
    { discoveredFilms: [], seenIds: [] },
  );
});
...
test("more than 31 regeneration steps survive reload and retain exclusion IDs after snapshot trimming", () => {
  ...
  for (let step = 0; step < 70; step++)
    session = commitSnapshot(
      session,
      { ...snapshot(step), action: step ? "regenerate" : "initial" },
      step === 0,
    );
...
test("undo and branching keep all displayed exclusions including archived ancestors and abandoned future discoveries", () => {
  let session = EMPTY_SESSION;
  for (let step = 0; step < 70; step++)
    session = commitSnapshot(session, snapshot(step), step === 0);
  ...
  session = commitSnapshot(
    session,
    { ...snapshot(100), action: "manual" },
    false,
  );
  const after = discoveryHistoryContext(session, true);
  assert.deepEqual(
    session.snapshots.map((sn) => sn.id),
    ["step-6", "step-7", "step-100"],
  );
  ...
  assert.equal(
    discoveryHistoryContext(parseSession(JSON.stringify(session)), true).seenIds
      .length,
    852,
  );
  ...
  assert.deepEqual(
    discoveryHistoryContext(fresh, true).seenIds,
    snapshot(200).recommendations.map((rec) => rec.film.id),
  );
});
...
test("large saved explanations trim old snapshots without forgetting regeneration exclusions", () => {
  ...
  for (let step = 0; step < 10; step++) {
    ...
    sn.recommendations = sn.recommendations.map((rec) => ({
      ...rec,
      detailToken: "x".repeat(15000),
    }));
  ...
  assert.equal(
    parseSession(JSON.stringify(session)).snapshots.at(-1)?.id,
    "step-9",
  );
});

test("history capacity fails explicitly rather than silently making old films eligible", () => {
  const archivedSeenIds = Array.from(
    { length: MAX_SEEN_FILMS },
    (_, i) => `old-${i}`,
  );
  const session = { ...EMPTY_SESSION, archivedSeenIds };
  assert.throws(
    () => commitSnapshot(session, snapshot(0), false),
    /history limit/,
  );
  assert.deepEqual(session.snapshots, []);
  assert.deepEqual(
    commitSnapshot(session, snapshot(0), true).snapshots.map((sn) => sn.id),
    ["step-0"],
  );
});
...
test("v2 saved paths without new action or curation fields remain readable", () => {
  ...
  assert.deepEqual(
    discoveryHistoryContext(restored, true).seenIds,
    [...snapshot(0).recommendations, ...snapshot(1).recommendations].map(
      (rec) => rec.film.id,
    ),
  );
});
  ...
  assert.throws(
    () =>
      commitSnapshot(
        ...
        {
          ...snapshot(0),
          recommendations: snapshot(0).recommendations.slice(0, 11),
        },
        true,
      ),
    /exactly twelve/,
