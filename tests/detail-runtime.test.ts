...
function isolate(t: { after: (fn: () => void) => void }) {
  ...
  Object.assign(process.env, {
    OPENAI_API_KEY: "test-key-no-live-network",
    PUBLIC_MODE: "true",
    VERCEL: "0",
    QUOTA_SERVICE_URL: QUOTA,
    QUOTA_SERVICE_SECRET: "test-quota-secret",
  });
  globalThis.fetch = async () => {
    throw new Error("Unexpected network call in detail tests.");
  };
  t.after(() => {
...
function token(label: string) {
  ...
  const rec: Recommendation = {
    ...
    curation: {
      lens: "Everyday rituals",
      bridge: "A formal connection.",
      contrast: "A different use of sound.",
    },
  };
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              paragraphs: [
                "A specific connection between the selected films.",
                "How this film changes that viewing question.",
                "A concrete formal detail to attend to.",
              ],
            }),
          },
        ],
...
test("production details accept 60 distinct expansions without a quota service or caller limit", async (t) => {
  ...
  await assert.rejects(
    explainFilm("forged.token", "en", new AbortController().signal),
    code("INVALID_INPUT"),
  );
  assert.equal(models, 0);
  for (let i = 0; i < 60; i++) {
    const request = new Request(
      "https://strada.example.net/api/recommendations/explain",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Origin: "https://strada.example.net",
          "x-vercel-forwarded-for": "192.0.2.1",
        },
        body: JSON.stringify({
          token: token(`Unlimited connection ${i}.`),
          language: "en",
        }),
      },
    );
    ...
    assert.equal((await response.json()).paragraphs.length, 3);
  }
...
test("shared detail viewers use one model call and completed details reuse the cache", async (t) => {
  isolate(t);
  let modelCalls = 0,
    resolveModel!: (response: Response) => void,
    started!: () => void,
  ...
  const signed = token("One explanation shared by simultaneous viewers."),
    a = new AbortController(),
    b = new AbortController();
  const first = explainFilm(signed, "en", a.signal),
    second = explainFilm(signed, "en", b.signal);
  ...
  assert.equal(value.paragraphs.length, 3);
...
test("last-viewer cancellation aborts model work and neither cancellation nor failure is cached", async (t) => {
  ...
  const signed = token("Cancellation and failure must remain retryable."),
    a = new AbortController(),
    b = new AbortController();
  const first = explainFilm(signed, "en", a.signal),
    second = explainFilm(signed, "en", b.signal);
  const firstCanceled = assert.rejects(first, { name: "AbortError" }),
    secondCanceled = assert.rejects(second, { name: "AbortError" });
  await ready;
  a.abort();
  await firstCanceled;
  assert.equal(modelSignal?.aborted, false);
  ...
  await assert.rejects(
    explainFilm(signed, "en", new AbortController().signal),
    code("RESEARCH_ERROR"),
  );
  const retried = await explainFilm(signed, "en", new AbortController().signal);
  assert.equal(retried.paragraphs.length, 3);
...
test("independent detail requests can run concurrently without the former two-request cap", async (t) => {
  ...
  const requests = Array.from({ length: 4 }, (_, i) =>
    POST(
      new Request("https://strada.example.net/api/recommendations/explain", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Origin: "https://strada.example.net",
        ...
        body: JSON.stringify({
          token: token(`Concurrent connection ${i}.`),
          language: "en",
        }),
      }),
    ),
