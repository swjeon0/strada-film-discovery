...
test("serialized recommendation tail is hidden while the explanation is retained", () => {
  for (const tail of [
    ...
    assert.equal(
      cleanDisplayProse(input),
      "Ordinary gestures make the passage of time tangible.",
      tail,
    );
  }
});
...
test("Korean explanations survive curly quotation and schema-key contamination", () => {
  const input =
    "일상의 작은 몸짓과 시간이 쌓이는 방식을 살펴보세요.”, ”inferenceWhyKo”: {”connection”: ”다음 값”}, ”sourceIds”: []}";
  assert.equal(
    cleanDisplayProse(input),
    "일상의 작은 몸짓과 시간이 쌓이는 방식을 살펴보세요.",
  );
});
...
test("balanced human quotations before a machine tail keep their closing marks", () => {
  assert.equal(
    cleanDisplayProse('The image asks: “What is home?”, "sourceIds": []}'),
    "The image asks: “What is home?”",
  );
  assert.equal(
    cleanDisplayProse('The image asks: "What is home?", "sourceIds": []}'),
    'The image asks: "What is home?"',
...
test("cleaning is idempotent and can drive a safe saved-language fallback", () => {
  const goodEnglish =
    "Watch how a pause changes the relationship between the two characters.";
  const savedKorean = "”sourceIds”: []}]}";
  const display =
    cleanDisplayProse(savedKorean) || cleanDisplayProse(goodEnglish);
