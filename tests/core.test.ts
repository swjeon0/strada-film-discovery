...
test('additional, duplicate or unknown anchors earn no automatic relevance bonus',async()=>{const {connectionScore}=await import('../lib/ranking');const template=recommendCollection([film('closeup')],[]).recommendations[0];const edge=template.connections[0];const one={...template,connections:[edge]};const duplicates={...template,connections:[edge,edge,{...edge,anchorId:'unknown'}]};assert.equal(connectionScore(one,[film('closeup')],[]),connectionScore(duplicates,[film('closeup')],[]));const two={...template,connections:[{...edge,anchorId:'closeup',relation:'direct_connection' as const},{...edge,anchorId:'fake',relation:'direct_connection' as const}]};assert.equal(connectionScore(two,[film('closeup'),film('fake')],[]),1);});
