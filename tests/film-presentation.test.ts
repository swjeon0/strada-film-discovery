...
const search: Film = {
  ...
  year: 2000,
  director: "",
  poster: "https://image.tmdb.org/t/p/w500/korean-poster.jpg",
};
const details: Film = {
  id: "canonical-film",
  ...
  poster: "/posters/english-poster.jpg",
  synopsisEn: "A verified synopsis.",
  genres: ["18"],
  titleKo: "검증된 한국어 제목",
  titleKoSource: "https://www.themoviedb.org/movie/123",
};
...
test("absent or rejected artwork falls back to valid details without discarding a DB Korean title", () => {
  for (const poster of ["", "https://evil.example/poster.jpg"]) {
    const selected = mergeFilmPresentation(
      { ...search, poster },
      { ...details, titleKo: undefined, titleKoSource: undefined },
    );
...
test("Korean curation uses the database title once without English-parenthetical duplicates", () => {
  ...
  const text =
    "Late Spring (만춘) and Late Spring(만춘) organize duration differently; 만춘 (만춘) is the same film.";
  assert.equal(
    localizeTitles(text, [film], "ko"),
    "만춘 and 만춘 organize duration differently; 만춘 is the same film.",
  );
  assert.equal(localizeTitles(text, [film], "en"), text);
  assert.equal(
    localizeTitles(
      "Late Spring preserves its DB name.",
      [{ ...film, titleKo: undefined }],
      "ko",
    ),
    "Late Spring preserves its DB name.",
  );
