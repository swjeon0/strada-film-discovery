...
- Knowledge: 원문 대조 검증을 거친 정규화 자료에서 빌드한 읽기 전용 검색 인덱스
...
추천 요청은 다음 흐름으로 처리됩니다.

1. 선택 영화의 메타데이터를 확인합니다.
2. `research/knowledge/serving-index.json`에서 선택 영화와 관련 맥락을 한 번 조회합니다.
3. 메인 큐레이터가 정확히 12편과 각 영화만의 짧은 연결 이유를 생성합니다. 전체를 하나의 논지로 묶거나 관계 유형별 비율을 강제하지 않습니다.
4. 응답이 생성되는 동안 완성된 영화부터 TMDB 또는 로컬 카탈로그로 제목·연도·감독을 병렬 확인합니다. 공개 결과에는 검증이 끝난 12편만 포함합니다.
5. 식별에 실패한 자리만 한 번 복구합니다. 이미 확인한 영화는 재사용하고 전체 목록의 중복·제외 조건을 다시 검증합니다.

목표 응답 시간은 20초입니다. 느린 외부 응답을 20초에서 바로 버리지 않도록 서버 작업은 최대 55초, Vercel 함수는 60초, 브라우저 전송은 65초로 제한합니다. 화면에는 서버에서 전달한 실제 진행 단계를 표시하며, 모델을 바꾸거나 결과를 줄여 시간을 맞추지 않습니다. 제공자 상태에 따라 20초를 넘을 수 있습니다.

같은 요청 ID와 본문으로 재접속하면 같은 실행을 공유합니다. 연결 종료 후 5초 동안 재접속을 기다리고, 성공 결과는 최대 32개·60초 동안 보관합니다. Vercel에서는 `after()`로 진행 중인 작업의 수명을 유지하되 재접속이 없으면 5초 뒤 취소합니다. 이 캐시는 실행 인스턴스 메모리에 있으므로 다른 인스턴스나 콜드 스타트 사이의 중복 방지를 보장하지 않습니다. 재생성은 새 요청 ID와 제외 목록으로 별도 수행합니다.
