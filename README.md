...
‘다른 영화 찾기’는 현재 경로에서 이전에 표시된 모든 추천을 제외하고 같은 선택으로 재생성합니다. 따라가기와 직접 추가는 직전12편 중 최대5편만 유지합니다. 부분 결과에서도 절반을 넘는 새 영화를 보장합니다. 성공한 요청만 스냅샷으로 저장하며, 재생성은 경로에 영화 한 편을 추가한 것으로 계산하지 않습니다. Undo, 경로 기록, 브라우저 이동은 저장된 정확한 스냅샷을 복원합니다. 최근 최대64개 스냅샷을 저장하고, 오래된 스냅샷을 줄여도 제외할 영화 ID는 보존합니다(최대5000개; 한도 도달 시 명시적으로 새 경로 안내).
